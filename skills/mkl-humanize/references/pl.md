...
Replace with the plain statement, or delete when the sentence carries no information.
...
| Nie da się ukryć, że… / Nie ulega wątpliwości, że… | Delete, or keep only if the claim is actually contested |
...
| umożliwia / pozwala na (as a reflex) | A verb: „podgląda”, „zapisuje”, „sprawdza” |
...
| szereg, wiele różnych, cały wachlarz | The number, or „kilka” |
...
- **„jest w stanie”** for every *can* → „może”, or just the verb.
- **Overused „że”-clauses after reporting verbs.** „Testy pokazują, że wynik jest
  lepszy” is fine; three in a row is a tell.
...
- Avoid inventing a gender for the author or reader. Impersonal forms („zrobiono”,
  „można zapisać”) and the first person plural („sprawdziliśmy”) are usually safe.
...
- `scripts/check_facts.py` counts the Polish negation and hedge words above, so it
  reports a dropped „nie” or „jeszcze”. It does not check aspect, case, or diacritics.
...
One suitable edit: „Dzięki za zgłoszenie. Żeby to sprawdzić, potrzebujemy wersji
systemu i Pythona oraz pełnego polecenia, które uruchomiłeś/uruchomiłaś.”
...
da się ukryć”, which asserted nothing, so the removal is intended and should be stated
as such rather than reverted. The gendered form is offered both ways because the reporter's gender is
unknown; an impersonal alternative is „pełnego uruchomionego polecenia”.
